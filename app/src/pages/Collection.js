...
function Collection() {
  ...
  //Go to the address page for an address within a specific collection
  const clickHandler = (addrs) => {
    navigate(`/${collection}/${addrs}`);
  };

  //Filter collections for where obj.slug is === collection
  useEffect(() => {
    ...
    //make call to backend server w/ axios to import transaction/wallet/trading data
    async function getCol() {
      ...
      setData(dataArray);
      //Amount held, avg days held, avg purchase price
      const bags = dataArray.map((a) => a.amount);
      const holds = dataArray.map((a) => a.avgHold);
      const prices = dataArray.map((a) => a.avgPrice);

      // const holdFilter = holds.filter(holds => holds !== undefined);
      // console.log("eliminate Undefined:", holdFilter);

      //Highest bag, highest avg hold, highest avg price
